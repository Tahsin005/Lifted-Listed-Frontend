...
const buyProduct = (event) => {
  ...
  const loadBalance = () => {
    return new Promise((resolve, reject) => {
      fetch(
        `https://lifted-listed-backend.onrender.com/user/account/${listedandlifted_user_account}/`
      )
        ...
        .then((data) => {
          ...
          fetch(`https://lifted-listed-backend.onrender.com/product/list/${product_id}/`)
            .then((res) => res.json())
            .then((prodata) => {
...
const reviewerDetails = async (user_id) => {
  try {
    const response = await fetch(
      `https://lifted-listed-backend.onrender.com/user/allUser/${user_id}/`
    );
...
const productDetailsHandler = async () => {
  ...
  try {
    ...
    const review = await fetch(
      `https://lifted-listed-backend.onrender.com/product/review/?product__id=${id}`
    );
